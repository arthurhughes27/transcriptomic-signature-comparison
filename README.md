...
- **`dearseq_dgsa.R`** / **`qusage_dgsa.R`** — loop `run_dearseq_comparison()` / `run_qusage_comparison()` over every valid comparison at each method's baseline specification (bolded options in Table 2.1), checkpointing results incrementally to `output/results/reanalysis/{method}_dgsa_results_list.rds`. Restricted by default to days 1, 3, and 7 (edit the `DAYS_TO_ANALYSE` constant near the top of each script — set to `NULL` for every timepoint present in the data); `02_run_raw_specifications.R` (below) has the same constant.
...
- **`04_specification_heatmaps.R`** — builds the two-tier robustness heatmap (`R/robustness_heatmaps.R` above): the aggregate-level heatmap (main figure) saved to `output/figures/specification_analysis/robustness_heatmap_aggregate.pdf`, and the gene-set-level heatmap (supplementary figure, height scaled to the number of gene sets shown) saved to `output/figures/specification_analysis/robustness_heatmap_genesets.pdf`. Safe to run against partial results from `03`.
